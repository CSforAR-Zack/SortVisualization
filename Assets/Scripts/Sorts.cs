...
public class Sorts : MonoBehaviour
{
    ...
    IEnumerator BubbleSortAlgorithm(GameObject[] unsortedList)
    {
        ...
        while(!done)
        {
            ...
            {
                SetColor(unsortedList[k], 2); // Current Place Color
                SetColor(unsortedList[k + 1], 4); // Looking At Color    
                yield return new WaitForSeconds(speed);
                ...
                if(unsortedList[k].transform.localScale.y > unsortedList[k + 1].transform.localScale.y)
                {
                    ...
                    yield return new WaitForSeconds(speed);
                    SetColor(unsortedList[k], 0); // Unsorted Color
                    done = false;
                }
                else
                {
            ...
            sorted++;
            SetColor(unsortedList[unsortedList.Length - sorted], 1); // Sorted Color
